 ...
 * Session-cookie protected. Phone-optimized. No password in client JS.
 */
...
export async function GET(request) {
  ...
  if (!validateSession(request.headers.get('cookie'))) {
    return new Response('<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0; url=/admin"></head><body>Redirecting to login...</body></html>', { status: 302, headers });
  }
...
.header{position:sticky;top:0;background:#050505;padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.15);z-index:10;display:flex;justify-content:space-between;align-items:center}
...
<body>
<div class="header"><a href="/admin">&larr; Dashboard</a><a href="/admin?logout=1">Logout</a></div>
...
codeInput.addEventListener('input', (e) => {
  ...
  searchTimer = setTimeout(async () => {
    ...
    try {
      const body = isCode ? {action:'checkin-lookup',code:val} : {action:'checkin-name',name:val};
      const res = await fetch(API, {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});
      const data = await res.json();
      if(!data.ok&&data.error==='not_found'){resultDiv.innerHTML='<div class="error">No RSVP found.</div>';return;}
      if(!data.ok&&data.error==='unauthorized'){window.location.href='/admin';return;}
      const rsvps = data.rsvp ? [data.rsvp] : (data.results || []);
      if(rsvps.length===0){resultDiv.innerHTML='<div class="error">No matches.</div>';return;}
      resultDiv.innerHTML = rsvps.map(r=>{
        const att=r.status==='ATTENDED';
        return '<div class="card"><div class="name">'+esc(r.guardian_name)+'</div>'+
          ...
          '<span class="status '+(att?'status-attended':'status-received')+'">'+r.status+'</span>'+
          (att?'':'<button class="btn-checkin" data-id="'+r.id+'">Mark as Attended</button>')+
        '</div>';
      }).join('');
      document.querySelectorAll('.btn-checkin').forEach(btn=>{
        btn.addEventListener('click', async()=>{
          ...
          const data=await res.json();
          if(data.ok){
            btn.parentElement.style.opacity='0.5';btn.textContent='Done!';
            if(navigator.vibrate)navigator.vibrate(100);
            setTimeout(()=>{codeInput.value='';resultDiv.innerHTML='';codeInput.focus();},800);
          } else {btn.disabled=false;btn.textContent='Try Again';}
        });
      });
    } catch{resultDiv.innerHTML='<div class="error">Connection error.</div>';}
  }, 300);
});
