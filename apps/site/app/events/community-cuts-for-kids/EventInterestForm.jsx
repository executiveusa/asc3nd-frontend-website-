...
export function EventInterestForm() {
  ...
  async function onSubmit(event) {
    ...
    const payload = {
      ...
      sourcePage: typeof window !== 'undefined' ? window.location.href : '/events/community-cuts-for-kids',
    };
  ...
  return (
    <form className={styles.form} onSubmit={onSubmit}>
      ...
      <p className={styles.formNote}>
        This form does not ask for children’s names, ages, stories, or other sensitive youth information.
      </p>

      {status.message ? (
        <p className={`${styles.status} ${styles[status.type] || ''}`} role="status">{status.message}</p>
      ) : null}
    </form>
  );
}
